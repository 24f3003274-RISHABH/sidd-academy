 ...
 * Moved to the absolute top of the middleware chain to catch preflight requests early.
 * Automatically sanitizes trailing slashes to prevent matching failures.
 */
const parseAllowedOrigins = () => {
  if (!ENV.CORS_ORIGIN || ENV.CORS_ORIGIN === '*') {
    return null;
  }
  return ENV.CORS_ORIGIN.split(',').map((o) => o.trim().replace(/\/$/, ''));
};
...
/**
 * WHY: Preflight Interceptor
 * Explicitly short-circuits and resolves incoming HTTP OPTIONS requests before payload parsers.
 */
...
/**
 * WHY: Body Parsers & Cookie Processing
 * Moved directly below the CORS configuration pipeline.
 */
...
// IMPORTANT FIX: Re-added missing default export required by server.js
export default app;
